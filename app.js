(() => {
  ...
  function sellerAssignmentSelect(attributes = "") {
    const options = customerDirectoryEntries()
      .map((entry) => `<option value="${esc(entry.key)}">${esc(entry.wechat)}${entry.phone ? ` · ${esc(entry.phone)}` : ""}</option>`)
      .join("");
    return `<select class="seller-inline-select" ${attributes} aria-label="选择送拍人"><option value="">待补送拍人 · 点击选择</option>${options}</select>`;
  }

  function repairUnscopedBuyerBackfillImport() {
    const entry = state.history.find((item) => item?.action === "回补本期买家资料" && Array.isArray(item?.before?.records));
    const beforeRecords = entry?.before?.records || [];
    const extraCount = state.records.length - beforeRecords.length;
    if (!entry || extraCount < 50) return {removed:0,matched:0};
    saveRecoveryCopy("repair-unscoped-buyer-backfill", {records:state.records,assets:state.assets});
    const matchingOrders = state.records.filter((incoming) => beforeRecords.some((existing) => MxiqiWorkflow.sameAuctionLot(existing, incoming)));
    const merged = MxiqiWorkflow.mergePlatformOrderRecords(clone(beforeRecords), matchingOrders, new Date().toISOString());
    state.records = merged.records;
    state.assets = Array.isArray(entry.before.assets) ? clone(entry.before.assets) : state.assets;
    syncStoredAssetsFromRecords();
    state.assets = MxiqiAssets.rematchAssets(state.assets, state.records);
    return {removed:extraCount,matched:merged.matched};
  }
  ...
  function renderRecordRow(record, child = false) {
    ...
    const hasConsignor = MxiqiWorkflow.hasConsignorName(record.sellerWechat);
    const resultPending = isAuctionResultPending(record);
    return `<tr class="${state.selected.has(record.id) ? "selected-row" : ""}${child ? " package-child-row" : ""}">
      ...
      <td class="role-cell"><span class="role-label">送拍人 / 结算对象</span>${hasConsignor ? `<button class="consignor-link" type="button" data-customer-open="${esc(consignor.key)}"><b>${esc(consignor.wechat || "待补")}</b><small>${esc(consignor.phone || record.sellerPhone || "送拍人手机号待补")}</small></button>` : `${sellerAssignmentSelect(`data-seller-assign-record="${esc(record.id)}"`)}<small>从已保存档案中选择</small>`}<small>${esc(record.trackingNumber || "未填寄入快递单号")}</small></td>
  ...
  function renderPackageRow(group) {
    ...
    records.forEach((record) => {
      if (!MxiqiWorkflow.hasConsignorName(record.sellerWechat)) return;
      const entry = consignorDirectoryEntry(record);
      if (entry.key) sellerMap.set(entry.key, entry);
    });
    ...
    return `<tr class="package-summary-row${allSelected ? " selected-row" : ""}">
      ...
      <td class="role-cell"><span class="role-label">送拍人 / 结算对象</span>${sellers.length === 1 ? `<button class="consignor-link" type="button" data-customer-open="${esc(sellers[0].key)}"><b>${esc(sellerSummary)}</b><small>${esc(sellers[0].phone || "送拍人手机号待补")}</small></button>` : sellers.length ? `<b>${esc(sellerSummary)}</b><small>展开后按拍品查看</small>` : `${sellerAssignmentSelect(`data-package-seller-assign="${esc(group.key)}"`)}<small>为本包裹全部 ${records.length} 件设置</small>`}</td>
  ...
  function renderPreauctionRow(record) {
    const consignor = consignorDirectoryEntry(record);
    const hasConsignor = MxiqiWorkflow.hasConsignorName(record.sellerWechat);
    return `<tr class="preauction-row"><td>${hasConsignor ? `<button class="consignor-link" type="button" data-customer-open="${esc(consignor.key)}"><b>${esc(consignor.wechat || "待补送拍人")}</b><small>${esc(consignor.phone || "手机号待补")}</small></button>` : `${sellerAssignmentSelect(`data-seller-assign-record="${esc(record.id)}"`)}<small>从已保存档案中选择</small>`}</td><td><span class="preauction-lot">${esc(record.lot)}</span></td><td><button class="preauction-item-button" type="button" data-action="edit" data-id="${esc(record.id)}">${esc(record.itemName)}</button></td><td><b>${esc(auctionPeriod(record))}</b></td></tr>`;
  }
  ...
  async function runSettlementSync({period = state.filters.auction, automatic = false, buyersOnly = false} = {}) {
    ...
    try {
      ...
      const scopedPlatformOrders = buyersOnly
        ? platformOrders.filter((incoming) => state.records.some((existing) => auctionPeriod(existing) === period && MxiqiWorkflow.sameAuctionLot(existing, incoming)))
        : platformOrders;
      const orderMerge = MxiqiWorkflow.mergePlatformOrderRecords(state.records, scopedPlatformOrders, timestamp);
  ...
  $("#records-body").addEventListener("change", (event) => {
    ...
    if (sellerRecordId || sellerPackageKey) {
      ...
      syncStoredAssetsFromRecords();
      rematchAssetsAndApply();
      audit("关联送拍人", `${entry.wechat} · ${records.length} 件拍品`);
      state.selected.clear();
      render();
      notify(`已把 ${records.length} 件拍品关联给 ${entry.wechat}`);
      return;
    }
  ...
  async function exportChecklistImage(records, options) {
    ...
    const columns = [
      ...
      {label:"拍品名称",x:398,width:650},
      {label:"拍卖期数",x:1048,width:160},
      {label:"拍卖日期",x:1208,width:210},
      {label:"送拍人手机号",x:1418,width:234},
    ];
    context.fillStyle = "#e8e5dd";
    context.fillRect(36, headerHeight, width - 72, rowHeight);
    context.fillStyle = "#52636c";
    ...
    sorted.forEach((record, index) => {
      ...
      context.font = '17px "Microsoft YaHei", sans-serif';
      const identity = consignorDirectoryEntry(record);
      const auctionDate = MxiqiWorkflow.auctionDateKey(record.auctionAt || record.platformOrderDate) || datePart(record.auctionAt || record.platformOrderDate) || "日期待补";
      const values = [truncate(identity.wechat || "待补送拍人",12),record.lot,truncate(record.itemName,34),auctionPeriod(record),auctionDate,identity.phone || record.sellerPhone || "手机号待补"];
      values.forEach((value, columnIndex) => context.fillText(String(value), columns[columnIndex].x + 10, y + 37));
    });
    context.fillStyle = "#7b898f";
    context.font = '14px "Microsoft YaHei", sans-serif';
  ...
  const startupBuyerScopeRepair = repairUnscopedBuyerBackfillImport();
  if (startupBuyerScopeRepair.removed) {
    audit("自动清理误导入订单", `移除 ${startupBuyerScopeRepair.removed} 条非当前工作台订单，保留并回补 ${startupBuyerScopeRepair.matched} 件原有拍品`, {undoable:false});
  }
  ...
  } else if (startupBuyerScopeRepair.removed) {
    notify(`已自动清理 ${startupBuyerScopeRepair.removed} 条误导入订单，工作台已恢复为原有拍品`);
